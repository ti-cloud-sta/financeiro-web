...
- **Prevenção de Duplicidade Simples:** O sistema validava a existência baseando-se na chave composta de `Unidade + Série + Título + Parcela`. Se a pendência já existisse com estes atributos, a linha era ignorada para evitar duplicação em banco.
