...
- **Regras de Negócio e Integridade (Colaboradores e Importações)**:
  ...
  - **Importações de Inadimplência (Pendências)**:
    - **Aba Única de Importação (Resumo)**: A importação de planilhas deve ler exclusivamente a aba `Resumo` (`sheet_name="Resumo"`). Não ler nem verificar outras abas da planilha.
    ...
    - **Unicidade e Diferenciação por Espécie e Carteira**: A identificação de pendências existentes considera a combinação de `Unidade`, `Série`, `Título`, `Parcela`, `Espécie` e `Carteira`. O mesmo título pode conter múltiplas linhas com espécies diferentes (ex: `DP` e `AD`) ou carteiras distintas; o sistema deve importá-las como registros independentes no banco de dados, sem sobrescrita mútua ou descarte por duplicidade.
    - **Atualização de Saldo e Carteira na Reimportação**: Quando um título já existente reaparece na importação:
      - Se o `valorSaldo` for alterado (ex: amortização/abatimento), ele deve ser atualizado independentemente da data de vencimento e gerar um histórico de "Atualização de Saldo".
      - Se a `carteira` (coluna M) for alterada, ela deve ser atualizada, os campos `fase` e `status` manuais devem ser limpos (`None`) para que a view `vw_nfpendencias_fase` reclassifique o título automaticamente, e históricos de "Atualização de Carteira" e "Classificação" devem ser gerados.
      - Se a data de vencimento sofrer ajuste (`venc_excel != venc_db`) que não configure prorrogação, a data é atualizada com histórico.
    - **Prorrogações (Resolução de Pendência)**: Ao reimportar uma planilha, se uma pendência já existir no banco e a nova data de vencimento for maior (`venc_excel > venc_db`), ela é considerada prorrogada. O registro deve ser atualizado (`dtVencimento`), marcado como prorrogado (`encerrado = 'P'`) e dois históricos gerados: "Título Prorrogado" e "Resolução".
    - **Baixa Automática por Ausência (Rastreamento por ID)**: Durante a leitura da planilha, todos os IDs de pendências processadas são rastreados (`ids_processados_planilha`). Pendências ativas no banco que não constarem no novo arquivo são automaticamente marcadas como encerradas (`encerrado = 'S'`) com o histórico de "Pendência finalizada" (assinado pelo usuário do sistema, ex: id 14).
    ...
    - **Opções de Status Padronizadas**: Todos os select boxes de status (filtro Kanban, modal de detalhes e modal de mudança de fase por drag-and-drop) devem conter opções padronizadas em ordem alfabética estrita (A-Z), incluindo expressamente `AD` e `AN`.
